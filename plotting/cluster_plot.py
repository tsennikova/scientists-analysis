...
filename =  os.path.join(data_for_plotting, '1-cluster_x-810-9.txt')  
with open(filename) as f:
    x_1 = f.read().splitlines()

x_1 = map(int, x_1)
x_1 = running_mean(x_1, 90)
f.close()

filename =  os.path.join(data_for_plotting, '1-cluster_y-810-9.txt')  
with open(filename) as f:
    y_1 = f.read().splitlines()

y_1 = map(float, y_1)
y_1 = running_mean(y_1, 90)
f.close()

filename =  os.path.join(data_for_plotting, '2-cluster_x-810-9.txt')  
with open(filename) as f:
    x_2 = f.read().splitlines()

x_2 = map(int, x_2)
x_2 = running_mean(x_2, 90)
f.close()


filename =  os.path.join(data_for_plotting, '2-cluster_y-810-9.txt')  
with open(filename) as f:
    y_2 = f.read().splitlines()

y_2 = map(float, y_2)
y_2 = running_mean(y_2, 90)
f.close()
...
#plt.show()
plt.savefig(plots_dir+'\\'+'clusters_views_3b_1a_seed-810-9.pdf')